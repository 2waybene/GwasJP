...
def common_variant_analysis_genotyped():
    ...
    for path in sys.argv[1:]:
        ...
        for i,pheno in enumerate(phenos):
            ...
            cmd = ' '.join(('sbatch -p standard -o '+path+'/sbatch_logs/mindcog.interaction.setup.'+pheno+'.out ./bin/jj.scripts/mindcog.interaction.model.setup.sh',
                                path,pheno,models[i],str(snplist)))
            print 'Setup mindcog ACR and APOE interaction models with',models[i],'model for phenotype',pheno+':\n',cmd            
            ## Split cmd for shell
