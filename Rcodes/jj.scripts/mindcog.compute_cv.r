...
models<-unlist(read.table(paste0(path,"/apoe/models.txt"),stringsAsFactors=F))
phenos<-sapply(strsplit(models,"~"),"[",1)

# Load phenotype
#source("bin/load_pheno_data.r")
pheno_data<-load_pheno_data(path,paste0(path,"/apoe/pheno_data.apoe.txt"))
head(pheno_data)
...
for (i in 1:length(phenos)) {
  file_list[[i]]<-file(paste0(path,"/apoe/association_cv/imputed_chunks/chr",chr,".",chunk,".",phenos[i],".assoc"),open="w")
...
p = c()
n = c()
coeffs = c("dosage","E21","E41","E21:dosage","E41:dosage")
#read.range <- 1
for (read_i in read.range) {
 ...
 #cat('am I here?1')    
    ...
    if (geno_data_info[geno_data_i]) {
      ...
      for (i in 1:length(phenos)) {
	#cat('am I here?1')
        f<-as.formula(paste0(models[i],"+dosage+E2*dosage+E4*dosage"))
