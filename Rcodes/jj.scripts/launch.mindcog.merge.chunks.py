...
def collapse_imputed_chunks():
	...
	print chroms
        ...
        for i,pheno in enumerate(phenos):
            for chrm in chroms:                
                #cmd = ' '.join(('sbatch -p long -o '+path+'/sbatch_logs/chr0.'+pheno+'.%j.out ./bin/model.eval.cv.genotyped.sh',
                #path,pheno,models[i]))
                cmd = ' '.join(('sbatch -p long -o '+path+'/sbatch_logs/chr'+str(chrm)+'mindcog.merge.chunks.'+pheno+
                                '.out ./bin/jj.scripts/mindcog.merge.chunks.callR.sh',
                                path,pheno,str(chrm)))
