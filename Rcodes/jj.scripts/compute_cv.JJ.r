...
for (read_i in read.range) {
  ...
  ####################
    ...
    #obj<-try(geno_data<-fread(f_gz,header=F,nrow=read_size,skip=read_i*read_size))
  ...
  geno_data<-as.matrix(geno_data[,-c(1:5)])
    ...
    if (length(which(snp_data$V4==TRUE))>0){
        snp_data$V4[which(snp_data$V4==TRUE)] <- "T"
    }
    if (length(which(snp_data$V5==TRUE))>0){
        snp_data$V5[which(snp_data$V5==TRUE)] <- "T"
    }
  ...
  ####################
    ...
    genos <- rep('0 0',length(geno_data)/3)
    length(genos)
    head(genos)
    head(geno_data)[1:30]
    head(geno_0_idx)
    geno.idx <- 1:(length(geno_data)/3)
    tmp.a1 <- snp_data$V4
    tmp.a2 <- snp_data$V5
    ...
    for (i in geno.idx){
        ## Major homo
        if (geno_data[geno_0_idx[i]]>.5){
            genos[i] <- paste0(tmp.a1," ",tmp.a1)
        }else if(geno_data[geno_1_idx[i]]>.5){
            genos[i] <- paste0(tmp.a1," ",tmp.a2)
        }else if(geno_data[geno_2_idx[i]]>.5){
            genos[i] <- paste0(tmp.a2," ",tmp.a2)
        }else{
            print(i)
