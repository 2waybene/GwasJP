...
dostuff = function(subpath){
    file.list <- list.files(file.path(path,subpath,"association_cv"))
    ...
    for(f in merge.files){
        ...
        temp.file <- try(fread(file.path(path,subpath,"association_cv",f),header=TRUE),silent = TRUE)
        if(any(class(temp.file)!="try-error")){
            if(which(merge.files==f)==1){ ## This overwrites the current file
                write.table(temp.file,file.path(path,subpath,"association_cv",out.name),row.names=FALSE)
            }else{
                write.table(temp.file,file.path(path,subpath,"association_cv",out.name),row.names=FALSE,col.names=FALSE,append=TRUE)
            }              
        }
    }
}
