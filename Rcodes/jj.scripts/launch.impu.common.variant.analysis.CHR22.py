...
def common_variant_analysis_imputed():
	...
	chroms = list(range(22, 23, 1))# + list(range(101, 113, 1))
	...
	print chroms
        ...
        for i,pheno in enumerate(phenos):
            for chrm in chroms:
		## Impute Analysis
                 cmd = ' '.join(('sbatch -p standard -o '+path+'/sbatch_logs/chr'+str(chrm)+'.cv.'+pheno+
		...
		## Impute Analysis for Meta Analysis
		 cmd = ' '.join(('sbatch -p standard -o '+path+'/sbatch_logs/chr'+str(chrm)+'.cv.for.meta.'+pheno+
