...
file.list <- list.files(file.path(path,"association_cv","imputed_chunks"))
...
for(f in merge.files){
    ...
    temp.file <- try(read.table(file.path(path,"association_cv","imputed_chunks",f),header=TRUE,sep='\t'),silent = TRUE)

    if(class(temp.file)!="try-error"){
        out.file <-rbind(out.file,temp.file)
    }else{
	cat('**ERROR** Potential Error, try-error, read error for',file.path(path,"association_cv","imputed_chunks",f),'\n')	
    }
}
if (debug){
    print('Printing...')
...
write.table(out.file,file.path(path,"association_cv",out.name),row.names=FALSE)
cat("Done\n")
