...
if (debug){
    path <- '~/NCSU_Projects/Accord/Data/analysis/Metformin/met_90d_allraces4'#'~/NCSU_Projects/Accord/accord_data/analysis/Metformin/met_90d_allraces4_intArm'
    setwd('~/NCSU_Projects/Accord/Data/analysis')#setwd('~/NCSU_Projects/Accord/accord_data/analysis')
    chr<-19
    chunk<-11
    f_gz <- "~/NCSU_Projects/Accord/accord_data/imputation/outputs/chr19.imputed.11"#'/tmp/accord.party.8.4/chr8.imputed.4'
	...
	#cat('am I here?1')
	  ...
	  #cat('am I here?')
        ...
        cat(paste0(chr,"\t",snp_data[snp_i,2],"\t",snp_data[snp_i,3],"\t",
                   snp_data[snp_i,4],"\t",snp_data[snp_i,5],"\t",maf,"\t",hwe_chi2,"\t",beta,"\t",se,"\t",p,"\t",n,"\n"),file=file_list[[i]])
        #print(summary(fit))
      }
    }
