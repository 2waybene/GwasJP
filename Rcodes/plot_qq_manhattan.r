...
GenAbleInstall <- FALSE
if(GenAbleInstall == TRUE){
  # Download and Install package traball from CRAN archive
  url <- "https://cran.r-project.org/src/contrib/Archive/GenABEL/GenABEL_1.8-0.tar.gz"
  pkgFile <- "GenABEL_1.8-0.tar.gz"
  download.file(url = url, destfile = pkgFile)
  
  
  url2 <- "https://cran.r-project.org/src/contrib/Archive/GenABEL.data/GenABEL.data_1.0.0.tar.gz"
  pkgFile2 <- "GenABEL.data_1.0.0.tar.gz"
...
library(scales)
library(GenABEL)#install.packages('GenABEL') estlambda(), 
debug <- FALSE
#debug <- TRUE
if (debug){
    #path <- '~/data/accord/data/analysis/Sulfonylurea/sulf_90d_black2'
    # path <- '~/NCSU_Projects/Accord/accord_data/analysis/Sulfonylurea/sulf_90d_allraces2'
    # path <- '/home/accord/data/analysis/tzd_weight_gain/tzd_allraces'
    path <- '/home/accord/data/analysis/simvastatin_targeted/simvastatin_targeted_onSimvaOnly'
...
lambda <- estlambda(data[,"P_unadj"])$estimate
#print(gc)
#gc <- gc$estimate
## Dataput info
...
cat("Phenotype:         ",pheno,"\n",
    #"MAF cutoff:        ",maf_cutoff,"\n",
     ...
     "Genomic inflation: ",format(lambda,digits=4),"\n",
     "Genotyped:         ",nrow(data[which(data$Type=='GENO'),]),"\n",
     "Imputed (all):           ",nrow(data[which(data$Type=='IMPU'),]),"\n",
     "Meta:           ",nrow(data[which(data$Type=='META'),]),"\n",
     sep="",file=output)
close(output)
gc()
...
if(lambda >1){
  ...
  #head(chi2_stats)
  chi2_stats<-chi2_stats/lambda
  adjP<-pchisq(chi2_stats,df=1,lower.tail=F)
  #plot(adjP)
  data$P_adj <- adjP
  gc()
}else{
  data$P_adj <- data$P_unadj
}

length(which(data$P_unadj==1))
#length(which(is.na(data$P_adj==NaN)))
#log(1)
min(data$P_adj)
...
# File for LocusZoom
metal_data <- subset(data, select = c("SNP", "P_unadj","P_adj"))
write.table(metal_data,paste0(path,"/association_cv/allChrom_",pheno,"_gc_adj_METAL.tsv"),
            row.names=FALSE,
            quote = FALSE,
            sep = "\t")
...
for (i in 1:length(chr)) {
  ...
  if(x_end == "-Inf"){
    print(paste0("No Data for CHR:", chr[i]))
    x_end <- temp + (50-chr[i])*10000000
    }
...
print("Printing unadjusted Manhattan plot")
## UNADJUSTED
if(debug){
    pt <- proc.time()
}
png(paste0(path,"/outputs/",pheno,"_manhattan.unadjusted.png"),width=8,height=4,units="in",res=300)
par(mar=c(4,4,2,1))
# Paint background
...
png(paste0(path,"/outputs/",pheno,"_manhattan.adjusted.png"),width=8,height=4,units="in",res=300)
par(mar=c(4,4,2,1))
# Paint background
